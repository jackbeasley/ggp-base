...
public class ShrekComboPlayer extends StateMachineGamer {
	List<Role> roles;
	...
	private int evalfn(Role role, MachineState state)
		...
		return goalProximity(role, state) * 1
				+ getOpponentProximity(role, state) * 1
				+ getOpponentMobility(role, state) * 1
				+ getMobility(role, state) * 1
				+ getOpponentMobility(role, state) * 1;
	}

	//goal proximity heuristic
	private int goalProximity(Role role, MachineState state) throws MoveDefinitionException, GoalDefinitionException{
		return getStateMachine().getGoal(state, role);
	}

	private int getOpponentProximity(Role role, MachineState state)
				throws MoveDefinitionException, GoalDefinitionException {
		StateMachine machine = getStateMachine();
		...
		for(Role curRole : roles){
			if(!curRole.equals(role)){
				proximity += goalProximity(curRole,state);
			}
		}
		return proximity / roles.size();
	}

	private int getOpponentMobility(Role role, MachineState state) throws MoveDefinitionException{
		StateMachine machine = getStateMachine();
		List<Role> roles = machine.getRoles();
		int mobility = 0;
		for(Role curRole : roles){
			if(!curRole.equals(role)){
				mobility += 100 - getMobility(curRole,state);
			}
		}
